...
def _load_box_calibration(raw_config: Dict[str, Any], config_path: Path) -> None:
    ...
    if not bool(section.get("enabled", False)) and not reference_needed:
        return
    ...
    def _finish_locked(
        ...
        *,
        ...
        if output_dir is not None and save_flags["geometry_result"]:
            ...
            side_entry_summary = scene_clean.get("m39_4_2_side_entry_validation") if isinstance(scene_clean, Mapping) else None
            if isinstance(side_entry_summary, Mapping):
