...
## M38.2 分支 B：部分开口与局部外圆柱联合拟合

M38.2 面向开口不完整、但仍能观察到部分端部和开口后方外圆柱侧面的半侧躺/侧躺实例。算法先由局部侧面法向、点云 PCA 和部分 mouth 方向生成轴线初值，再对已知外半径执行有界解析圆柱拟合；随后必须通过部分端部、开口中心、mouth 重叠率、近端方向和轴线视角门限，才允许在已定位开口平面上投影名义内外轮廓，恢复壁厚并复用原有内外夹持、钟点搜索和全碰撞检查。只看到外侧壁、没有开口/端部证据的实例不会被接受。

当前顺序为：全局 M38.1 → 全局 M38.2 → 按深度层回退 M36/M37.6。原 M36 和 M37.6 均保留。生产开关为 `m38_branch_b.enabled`，独立回放脚本为 `production/foam_ring_grasp/scripts/replay_m38_2_branch_b.py`。完整说明见 `docs/M38.2_PARTIAL_OPENING_LOCAL_CYLINDER.md`。
