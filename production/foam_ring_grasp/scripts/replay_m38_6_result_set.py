#!/usr/bin/env python3
"""Replay saved RGB-D bundles through the M38.6 A/B/D/C policy.

M38.6 branch D returns pure-side outer-contact geometry only. It intentionally
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("/tmp/m38_6_replay"))
...
def main() -> int:
    ...
    for bundle in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        axial_position = outer.get("axial_position") if isinstance(outer.get("axial_position"), Mapping) else {}
        robot_candidate = scene.get("robot_candidate") if isinstance(scene.get("robot_candidate"), Mapping) else {}
        robot_target = robot_candidate.get("target") if isinstance(robot_candidate.get("target"), Mapping) else {}
        row = {
            ...
            "terminal_message": branch_c.get("reason"),
            "terminal_display_reason": branch_c.get("display_reason_short"),
            "collision_check_performed": bool(branch_c.get("collision_check_performed", False)),
            "selected_clock_hour": robot_target.get("clock_hour"),
            ...
            "m38_5_contact_axial_policy": axial_position.get("policy"),
            "m38_5_contact_fraction_from_camera_near_end": axial_position.get("fraction_from_camera_near_end"),
            ...
            "m38_6_replay_wall_ms": wall_ms,
        }
        rows.append(row)
        overlay = draw_overlay(rgb, instances, scene, intrinsics)
        cv2.putText(
            overlay,
            f"M38.6 {row['selected_grasp_branch']} {wall_ms:.0f} ms",
        ...
        cv2.imwrite(str(output / f"{capture_id}_m38_6_overlay.jpg"), overlay)
        write_json(output / f"{capture_id}_m38_6_result.json", _strip_debug(scene))
    ...
    write_json(output / "summary.json", {"stage": "M38.6_direction_collision_contact_fix", "rows": rows})
    print(json.dumps({"stage": "M38.6", "captures": len(rows), "output": str(output)}, ensure_ascii=False, indent=2))
    return 0
