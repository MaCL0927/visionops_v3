#!/usr/bin/env python3
"""Replay M36/M37.4 hybrid selection on saved online debug bundles."""
...
def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay saved bundles with M37.4")
    parser.add_argument("root", type=Path, help="directory containing capture-id bundle directories")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("production/foam_ring_grasp/config/line.yaml"),
    )
    parser.add_argument("--output", type=Path, default=Path("/tmp/m37_4_replay"))
...
def main() -> int:
    ...
    for bundle in bundles:
        ...
        row = {
            ...
            "selected_depth_rank": layering.get("selected_depth_rank"),
            "m37_fast_attempt_count": side.get("fast_attempt_count"),
            "m37_local_refinement_count": side.get("accurate_refinement_count"),
            "m37_global_accurate_used": side.get("global_accurate_search_used"),
    ...
    write_json(
        ...
        {"stage": "M37.4", "count": len(rows), "rows": rows},
    )
    if rows:
