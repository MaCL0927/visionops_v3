#!/usr/bin/env python3
"""M37.5 acceptance test for the persistent hybrid foam-ring trigger service."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify M37.5 depth-layered hybrid M36/M37 persistent trigger service")
    ...
    for index in range(max(1, int(args.samples))):
        ...
        if result.get("robot_ready") is not False:
            failures.append(f"{request_id} robot_ready must remain false in M37.5")
    ...
    if failures:
        print("[FAIL] M37.5 acceptance failed:", file=sys.stderr)
        for failure in failures:
            print(f"  - {failure}", file=sys.stderr)
        return 2
    print("[PASS] M37.5 hybrid persistent trigger service is healthy.")
    return 0
