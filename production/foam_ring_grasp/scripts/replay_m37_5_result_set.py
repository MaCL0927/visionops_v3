#!/usr/bin/env python3
"""Replay saved debug bundles with M37.5 pose-safe hybrid selection."""
...
def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay saved bundles with M37.5")
    ...
    parser.add_argument("--output", type=Path, default=Path("/tmp/m37_5_replay"))
...
def main() -> int:
    ...
    write_json(args.output / "summary.json", {"stage": "M37.5", "count": len(rows), "rows": rows})
    if rows:
