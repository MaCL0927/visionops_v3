#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SOURCE_DIR="${ROOT}/production/detergent_grasp"
CONFIG_DIR="/etc/visionops_v3"
SYSTEMD_DIR="/etc/systemd/system"
CONFIG_FILE="${CONFIG_DIR}/detergent_grasp.yaml"
ENV_FILE="${CONFIG_DIR}/detergent_grasp.env"
ENABLE=1

if [[ "${1:-}" == "--no-enable" ]]; then
  ENABLE=0
elif [[ $# -gt 0 ]]; then
  echo "Usage: sudo bash production/detergent_grasp/deploy/install_services.sh [--no-enable]" >&2
  exit 2
fi

install -d -m 0755 "${CONFIG_DIR}"
if [[ ! -f "${CONFIG_FILE}" ]]; then
  install -m 0644 "${SOURCE_DIR}/config/line.yaml" "${CONFIG_FILE}"
  echo "Created ${CONFIG_FILE}. Check class IDs/names and video.public_url before robot integration."
else
  install -m 0644 "${SOURCE_DIR}/config/line.yaml" "${CONFIG_FILE}.example"
  echo "Kept existing ${CONFIG_FILE}; refreshed ${CONFIG_FILE}.example."
fi
if [[ ! -f "${ENV_FILE}" ]]; then
  install -m 0644 "${SOURCE_DIR}/deploy/production.env.example" "${ENV_FILE}"
fi

VENV_DIR="${VISIONOPS_VENV:-${ROOT}/venv}"
if [[ ! -x "${VENV_DIR}/bin/python3" ]]; then
  echo "VisionOps v3 venv not found: ${VENV_DIR}/bin/python3" >&2
  echo "Run first: sudo bash ${ROOT}/scripts/setup_edge_env.sh" >&2
  exit 1
fi

UNITS=(
  visionops-v3-detergent-grasp-runtime.service
  visionops-v3-detergent-grasp-app.service
  visionops-v3-detergent-grasp-collector.service
)
for unit in "${UNITS[@]}"; do
  install -m 0644 "${SOURCE_DIR}/deploy/systemd/${unit}" "${SYSTEMD_DIR}/${unit}"
done
systemctl daemon-reload
if [[ ${ENABLE} -eq 1 ]]; then
  systemctl enable --now "${UNITS[@]}"
  systemctl --no-pager --full status "${UNITS[@]}" || true
else
  echo "Installed only. Start with: systemctl enable --now ${UNITS[*]}"
fi
