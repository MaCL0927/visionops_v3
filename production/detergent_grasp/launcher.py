#!/usr/bin/env python3
"""Launch Runtime, App or Collector for the detergent-grasp profile."""
...
from production.detergent_grasp.config import DEFAULT_CONFIG_PATH, PROJECT_ROOT, load_config


def _config_path(value: Optional[str]) -> str:
    return str(Path(value or os.environ.get("VISIONOPS_DETERGENT_GRASP_CONFIG", DEFAULT_CONFIG_PATH)).expanduser())
...
def _runtime(config: dict) -> int:
    ...
    model_dir = Path(os.environ.get("VISIONOPS_DETERGENT_GRASP_MODEL_DIR", runtime["model_dir"]))
    ...
    active_model = str(config.get("active_camera", {}).get("camera_model") or bridge.get("camera_model") or "hp60c")
    ...
    command = [
        ...
        "--max-detections", str(int(runtime.get("max_detections", 100))),
...
def _app(config_path: str) -> int:
    command = [
        sys.executable,
        "-m",
        "production.detergent_grasp.tasks.detergent_grasp_vision.service",
        "--config",
        config_path,
    ]
    os.execv(command[0], command)
...
def _collector(config: dict) -> int:
    ...
    app = config["app"]
    os.environ["VISIONOPS_COLLECTOR_RUNTIME_SERVICE"] = "visionops-v3-detergent-grasp-runtime.service"
    os.environ["VISIONOPS_COLLECTOR_CAMERA_DEPENDENT_SERVICES"] = "visionops-v3-detergent-grasp-app.service"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="VisionOps detergent-grasp launcher")
    parser.add_argument("--config", help="默认 production/detergent_grasp/config/line.yaml")
