#!/usr/bin/env python3
"""Launch Runtime, App or Collector for the plastic-bag-grasp profile."""
...
from production.plastic_bag_grasp.config import DEFAULT_CONFIG_PATH, PROJECT_ROOT, load_config


def _config_path(value: Optional[str]) -> str:
    return str(Path(value or os.environ.get("VISIONOPS_PLASTIC_BAG_GRASP_CONFIG", DEFAULT_CONFIG_PATH)).expanduser())
...
def _runtime(config: dict) -> int:
    ...
    model_dir = Path(os.environ.get("VISIONOPS_PLASTIC_BAG_GRASP_MODEL_DIR", runtime["model_dir"]))
...
def _app(config_path: str) -> int:
    command = [
        sys.executable,
        "-m",
        "production.plastic_bag_grasp.tasks.plastic_bag_grasp_vision.service",
        "--config",
        config_path,
    ]
    os.execv(command[0], command)
...
def _collector(config: dict) -> int:
    ...
    app = config["app"]
    os.environ["VISIONOPS_COLLECTOR_RUNTIME_SERVICE"] = "visionops-v3-plastic-bag-grasp-runtime.service"
    os.environ["VISIONOPS_COLLECTOR_CAMERA_DEPENDENT_SERVICES"] = "visionops-v3-plastic-bag-grasp-app.service"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="VisionOps plastic-bag-grasp launcher")
    parser.add_argument("--config", help="默认 production/plastic_bag_grasp/config/line.yaml")
