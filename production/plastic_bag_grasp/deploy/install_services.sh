...
SOURCE_DIR="${ROOT}/production/plastic_bag_grasp"
CONFIG_DIR="/etc/visionops_v3"
SYSTEMD_DIR="/etc/systemd/system"
CONFIG_FILE="${CONFIG_DIR}/plastic_bag_grasp.yaml"
ENV_FILE="${CONFIG_DIR}/plastic_bag_grasp.env"
ENABLE=1

if [[ "${1:-}" == "--no-enable" ]]; then
  ENABLE=0
elif [[ $# -gt 0 ]]; then
  echo "Usage: sudo bash production/plastic_bag_grasp/deploy/install_services.sh [--no-enable]" >&2
  exit 2
fi
...
if [[ ! -f "${CONFIG_FILE}" ]]; then
  ...
  echo "Created ${CONFIG_FILE}. Check class IDs/names and video.public_url before robot integration."
else
...
UNITS=(
  visionops-v3-plastic-bag-grasp-runtime.service
  visionops-v3-plastic-bag-grasp-app.service
  visionops-v3-plastic-bag-grasp-collector.service
)
