#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""HTTP + WebSocket service for the plastic-bag bbox-centre grasp task.
...
from production.plastic_bag_grasp.config import DEFAULT_CONFIG_PATH, load_config
from production.plastic_bag_grasp.tasks.plastic_bag_grasp_vision.algorithm import (
    PlasticBagGraspAlgorithm,
    PlasticBagGraspResult,
)
from production.plastic_bag_grasp.tasks.plastic_bag_grasp_vision.depth_coordinate import DepthCoordinateClient
from production.plastic_bag_grasp.tasks.plastic_bag_grasp_vision.websocket_server import (
    WebSocketJsonServer,
    WebSocketSession,
)
...
class ServiceState:
    ...
    def snapshot(self, websocket: WebSocketJsonServer) -> Dict[str, Any]:
        with self.lock:
            ...
            return {
                "schema_version": "1.0",
                "message_type": "plastic_bag_grasp_service_status",
                "status": "ok",
...
class PlasticBagGraspVisionService:
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        self.algorithm = PlasticBagGraspAlgorithm(config["algorithm"])
        self.depth = DepthCoordinateClient(
            config["camera_bridge"],
            config["algorithm"]["depth"],
            timeout_s,
        )
        self.inference_settings_path = Path(str(config["app"]["inference_settings_path"]))
        self.production_fps_lock = threading.Lock()
        self.production_inference_fps = float(config["app"].get("default_production_inference_fps", 30.0))
        ...
        self.debug_root = Path(str(debug.get("save_root", "/tmp/visionops_v3/plastic_bag_grasp/latest")))
    ...
    def inference_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "ok",
            "app_id": "plastic_bag_grasp_vision",
    ...
    def start(self) -> None:
        ...
        self.worker_thread = threading.Thread(target=self._inference_loop, name="plastic-bag-grasp-inference", daemon=True)
        self.postprocess_thread = threading.Thread(
            target=self._postprocess_loop, name="plastic-bag-grasp-postprocess", daemon=True
        )
        self.status_thread = threading.Thread(target=self._status_loop, name="plastic-bag-grasp-status", daemon=True)
    ...
    def _robot_message(
        ...
        frame_id: int,
        timestamp: float,
        result: Optional[PlasticBagGraspResult],
        latency_ms: float,
        request_id: Optional[object],
        trigger_task_id: Optional[object],
        ...
        document: Dict[str, Any] = {
            ...
            "task_id": str(self.config.get("task_id") or "plastic_bag_grasp"),
    ...
    def _complete_packet(self, packet: InferencePacket, dispatch: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # M40: the robot's authoritative grasp point is the bbox centre.
            # Camera XYZ is optional and must never suppress a valid RGB target.
            depth_debug: Dict[str, Any] = {"ok": True, "mode": "not_requested"}
            if result.items:
                depth_started = time.perf_counter()
                try:
                    samples, depth_debug = self.depth.sample(
                        [item["center_px"] for item in result.items],
                        result.image_width,
                        result.image_height,
                    )
                    for index, item in enumerate(result.items):
                        ...
                        position = sample.get("position_camera") if isinstance(sample, Mapping) else None
                        if bool(sample.get("depth_valid")) and isinstance(position, list) and len(position) >= 3:
                            item["position_camera"] = [
                    ...
                    depth_debug = {
                        "ok": False,
                        "mode": "depth_optional_failed",
                        "error": str(depth_error),
                    }
                    self.state.counters["optional_depth_failures"] += 1
                timing["depth_sample_deproject_ms"] = round(
            ...
            diagnostics = {
                "selected_target_count": len(result.items),
            ...
            visualization["plastic_bag_grasp"] = {
                "robot_items": result.items,
                "selected_detections": result.selected,
            ...
            visualization["plastic_bag_grasp"]["app_timing"] = timing
    ...
    def _save_debug_async(self, decision: Mapping[str, Any]) -> None:
        ...
        threading.Thread(
            ...
            args=(snapshot,),
            name="plastic-bag-grasp-debug-writer",
            daemon=True,
        ).start()
    ...
    def _draw_overlay(rgb_bytes: bytes, visualization: Mapping[str, Any], output_path: Path) -> None:
        ...
        if image is None:
            return
        metadata = visualization.get("plastic_bag_grasp") if isinstance(visualization.get("plastic_bag_grasp"), Mapping) else {}
        selected = metadata.get("selected_detections") if isinstance(metadata.get("selected_detections"), list) else []
        for target in selected:
            ...
            if isinstance(center, list) and len(center) >= 2:
                ...
                label = "plastic_bag {:.2f}".format(float(target.get("confidence") or 0.0))
                cv2.putText(
                    image, label, (point[0] + 6, max(18, point[1] - 6)),
...
class StatusHandler(BaseHTTPRequestHandler):
    server_version = "VisionOpsPlasticBagGrasp/1.0"

    @property
    def service(self) -> PlasticBagGraspVisionService:
...
def run(config: Mapping[str, Any]) -> int:
    ...
    service.start()
    thread = threading.Thread(target=server.serve_forever, name="plastic-bag-grasp-http", daemon=True)
    thread.start()
    ws = config["websocket"]
    print(
        "Plastic Bag Grasp Vision started: ws={}:{}{} http={}:{} runtime={} video={}".format(
            ws["listen_host"],
            ws["listen_port"],
            ws["path"],
            app["listen_host"],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="VisionOps plastic bag grasp WebSocket/HTTP app")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH), help="plastic_bag_grasp YAML")
    return parser
