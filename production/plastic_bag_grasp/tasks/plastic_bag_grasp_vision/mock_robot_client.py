#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Dependency-free robot client for plastic-bag-grasp integration testing."""
                ...
                try:
                    ...
                    if isinstance(document, dict):
                        ...
                        if document.get("type") == "detection":
                            print(
                                "[RESULT] request_id={} targets={} image={}x{}".format(
                                    document.get("request_id"),
                                    len(document.get("items") or []),
                                    (document.get("image") or {}).get("width"),
                                    (document.get("image") or {}).get("height"),
                                )
                            )
                            for item in document.get("items", []):
                                if not isinstance(item, dict):
                                    continue
                                print(
                                    "[ITEM] id={} class_id={} confidence={} px={} camera_xyz_mm={}".format(
                                        item.get("id"),
                                        item.get("class_id"),
                                        item.get("confidence"),
                                        item.get("center_px"),
...
def main() -> int:
    ...
    try:
        while True:
            command = input("输入 start/stop/trigger/task/q（直接回车=task）：").strip().lower() or "task"
            if command == "q":
                break
            if command not in {"start", "stop", "trigger", "task"}:
                print("unsupported command")
                continue
            document = (
                {"type": "trigger", "task_id": "plastic_bag_grasp", "request_id": request_id}
                if command == "task"
                else {"type": "control", "command": command, "request_id": request_id}
            )
