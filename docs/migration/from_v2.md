...
M10.1 已在 `SnapshotProvider` 中加入轻量 JPEG 编码：当 `StreamWorker` 已缓存最新 RGB888 帧时，`snapshot.jpg` 返回真实帧 JPEG；没有最新帧时才回退内置 Mock JPEG。M10.2 把现场稳定性作为重点：HP60C Bridge 与 V4L2 帧源需要同步更新 `latest_frame_id`、`frames_captured`、`snapshot_encoder` 和 `last_error`，并保证 GET/HEAD snapshot 行为稳定。Collector Web 仍然只通过 Runtime HTTP API 获取快照，不能直接访问 `/dev/videoX` 或 HP60C SDK Bridge。
