...
## M10 Runtime Frame Source

M10 将真实相机输入接入到 C++ Runtime 的 `stream_worker` 边界。Runtime 通过统一 `ImageBuffer` 向 `Preprocess -> RknnRunner -> Postprocess` 链路提供图像，HTTP 层、Collector Web、Gateway 和 Business App 均不直接访问相机设备。
