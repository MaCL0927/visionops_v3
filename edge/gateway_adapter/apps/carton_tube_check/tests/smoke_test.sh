...
PORT=19110
LOG_FILE="$(mktemp /tmp/visionops-carton-tube.XXXXXX.log)"
PID=""
...
python -c 'import socket; s=socket.socket(); s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1); s.bind(("127.0.0.1", 19110)); s.close()'
cd "${ROOT_DIR}"
python -m edge.gateway_adapter.apps.carton_tube_check.service \
  --host 127.0.0.1 --port "${PORT}" --upstream-kind file --mock-case ok \
...
wait "${PID}"
PID=""
echo "carton_tube_check Mock 冒烟测试通过"
