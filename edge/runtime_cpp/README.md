...
## Orbbec 原始 RGB 共享内存帧源

同机部署的 Orbbec 336L 推荐使用：

```bash
--frame-source shared_memory
--shared-memory-name /visionops_orbbec336l_rgb
...
```text
Bridge BGR → JPEG 编码 → HTTP → JPEG 下载 → OpenCV 解码 → RGB888
```

改为：

```text
Bridge SDK Color → POSIX 双缓冲共享内存 RGB888 → Runtime
```

`shared-memory-fallback-http=true` 时，共享内存尚未创建、协议头失效或写入端停止后，
Runtime 会使用已有 HTTP JPEG 快照作为兼容回退。HP60C 继续使用
`hp60c_bridge`，不会错误切换到 Orbbec 共享内存。
...
curl -s http://127.0.0.1:28085/api/runtime/status | jq '.frame_source | {
  ...
  fps,
  latest_frame_age_ms,
  shared_memory_sequence,
  shared_memory_retry_count,
  last_error
}'
```
...
共享内存帧不需要 JPEG 解码，因此 `infer_once.timing.decode_ms` 应为 `0` 或接近 0。
`capture_ms` 主要是等待新 sequence 和复制约一帧 RGB 数据的耗时。

## RKNN 输入输出缓冲区复用

真实 RKNN Runner 现在执行两项低风险优化：

1. `RknnInput` 直接引用预处理输出，删除 `preprocess.input → RknnInput` 的整帧 host
   copy；
2. 模型加载时按输出 tensor 元素数量预分配 float buffer，后续
   `rknn_outputs_get()` 使用 `is_prealloc=1`，后处理直接读取这些 buffer，不再每帧让
   RKNN 动态申请输出并复制到第二组 `RuntimeTensor.data`。

为兼容旧版 RKNN Runtime/driver，如果预分配 float 输出被拒绝，程序会自动退回旧的
动态输出路径，模型仍可运行。每次推理结果的 `debug` 中可确认：

```bash
curl -s -X POST http://127.0.0.1:28085/api/runtime/infer_once | jq '{
  timing,
  timing_detail,
  transport: .debug.preprocess_backend_active,
...
```text
host_input_copy_avoided=true
output_buffers_preallocated=true
```
