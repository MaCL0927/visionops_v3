...
#   VISIONOPS_PRODUCTION_INFERENCE_SOURCE, VISIONOPS_DEVICE_ID
...
PRODUCTION_INFERENCE_SOURCE="${VISIONOPS_PRODUCTION_INFERENCE_SOURCE:-runtime}"
...
exec "${PYTHON_BIN}" -m apps.collector_web.backend.main \
  ...
  --production-inference-source "${PRODUCTION_INFERENCE_SOURCE}" \
  --device-id "${DEVICE_ID}"
